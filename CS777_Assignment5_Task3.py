...
negative_ratio = WikinumberOfDocs / numberOfLines
positive_ratio = AUnumberOfDocs / numberOfLines



print("Positive ratio = {:.8f}\nNegative ratio = {:.8f}".format(positive_ratio,negative_ratio))
...
for i in range(num_iteration+1):
    gradientCost = featureRDD.map(lambda x:(x[1],x[2],np.dot(x[1],theta))).map(lambda x:np.array([max(0,negative_ratio*((1+x[1])/2)*(1 - x[2]*x[1])+positive_ratio*((1-x[1])/2)*(1 - x[2]*x[1])),0 if (x[1]*x[2] > 1) else (-x[1]*x[0])*(negative_ratio*((1+x[1])/2)+positive_ratio*((1-x[1])/2))])).treeAggregate(np.zeros(2),lambda x,y:np.add(x,y),lambda x,y:np.add(x,y),3)
    cost = gradientCost[0]
    gradient = gradientCost[1]
    print("step {} : cost={:.8f}, gradient[0] = {:.8f}, gradient[4999] = {:.8f}".format(i,cost,gradient[0],gradient[4999]))
    if (i > 0 and cost > last_cost):
        print("cost has reached its minimum, algorithm stop.")
        break;
    last_cost = cost
